...
describe("EtherGameFixed", function() {
    let EtherGameFixed, etherGameFixed, accounts;
    ...
    describe('Upon deployment', () => {
        it('Has a balance of zero', async () => {
            expect(await etherGameFixed.getBalance()).to.equal(0);
        })
    })
    describe('Testing the deposit function', () => {
        it('Allows user to send 1 ether', async () => {
            ...
            expect(await etherGameFixed.getBalance()).to.equal(ethers.utils.parseEther("1.0"));
        });
        it('Allows multiple users to send 1 ether', async () => {
            await etherGameFixed.deposit({value: ethers.utils.parseEther("1.0")});
            etherGameFixed.connect(accounts[1]);
            await etherGameFixed.deposit({value: ethers.utils.parseEther("1.0")});
        ...
        it('Selects a winner after 7 ether reaches the smart contract', async () => {
            ...
            expect(await etherGameFixed.winner()).to.equal(accounts[0].address);
        });
        it('Does not let user send more ether if 7 ether has been sent', async () => {
            ...
            try {
                ...
                expect(await etherGameFixed.getBalance()).to.equal(ethers.utils.parseEther("7.0"));
            }
        });
        it('Does not let user send more than 1 ether to the deposit function in a single call', async() => {
            try {
                await etherGameFixed.deposit({value: ethers.utils.parseEther("2.0")});
            } catch {
                expect(await etherGameFixed.getBalance()).to.equal(0);
            }
        });
    ...
    describe('Testing the claimReward function', () => {
        it('Does not allow user to claim reward if target is less than 7', async () => {
            await etherGameFixed.deposit({value: ethers.utils.parseEther("1.0")});
            try {
                await etherGameFixed.claimReward();
            } catch {
                expect(await etherGameFixed.getBalance()).to.equal(ethers.utils.parseEther("1.0"));
            }
        });
        ...
        it('Does not allow a different user to claim award', async () => {
            for (let i = 0; i < 7; i++) {
                await etherGameFixed.connect(accounts[0]);
                await etherGameFixed.deposit({value: ethers.utils.parseEther("1.0")});
            }
            await etherGameFixed.connect(accounts[1]);
            try {
                await etherGameFixed.connect(accounts[1]).claimReward();
            } catch {
                expect(await etherGameFixed.getBalance()).to.equal(ethers.utils.parseEther("7.0"));
            }
        });
